    ...
    return (
        <div className='container mx-auto px-4 my-6 relative'>
            ...
            >
                ...
                {/* map data products and Link to single product */}
                {
                    loading ?
                        loadingList.map((item, index) => (
                            <div
                                className='min-w-[280px] max-w-[280px] md:min-w-[320px] md:max-w-[320px] w-full
                                             h-36 bg-slate-200 rounded shadow-md animate-pulse'
                                key={index}></div>
                        )) : (
                            data.map((item, index) => {
                                return (
                                    // cart of product and Link to single product
                                    <Link
                                        to={`/product/${item?._id}`}
                                        key={index}
                                        className='min-w-[280px] max-w-[280px] md:min-w-[320px] md:max-w-[320px] w-full
                                                h-36 bg-white rounded-sm shadow flex'
                                        onClick={()=>window.scrollTo(0, 0)}>
                                        {/* image */}
                                        <div className='bg-slate-200 h-full p-4 min-w-[120px] md:min-w-[145px] flex
                                        ...
                                        <div className='p-4 grid'>
                                            ...
                                            <div className='flex gap-3 text-xs'>
                                                ...
                                                {/* price */}
                                                <p className='text-slate-500 line-through overflow-auto'>
